...
y_train <- exp(x_train+1)+3+epsilon_train  # train set
y_train_no_error <- exp(x_train+1)+3
...
y_test <- exp(x_test+1)+3+epsilon_test     # test set
...
lines(seq(-2.5,2.5,0.1), exp(seq(-2.5,2.5,0.1) + 1) + 3, col='black', lwd=2) # true relationship.. 
...
abline(linear_model,col="blue",lwd=3, lty=2) # linear relationship
lines(seq(-2.5,2.5,0.1), exp(seq(-2.5,2.5,0.1) + 1) + 3, col='black', lwd=2) # true relationship.. 
lines(train_data_sorted$x,kf2$fitted,col="magenta",lwd=3)

plot(x_train,y_train, main="Training Set: true Vs knn(k=12)", ylab = "Y Training", xlab = "X Training", col="orange",pch=19) 
abline(linear_model,col="blue",lwd=3, lty=2) # linear relationship 
lines(seq(-2.5,2.5,0.1), exp(seq(-2.5,2.5,0.1) + 1) + 3, col='black', lwd=2) # true relationship.. 
lines(train_data_sorted$x,kf12$fitted,col="magenta",lwd=3)
